...
data_root = Path(
    "/data00/shared/mahmudul/Sbu_Kyt_Pdac_merged/Codes/Tumor_Mirco_Env_Data/Div_Data/"
)
...
tm.run_spatial_analysis(
    patches, cells, microenv_distances=[100], mpp=0.34622, output_path="output.csv"
)
...
for point_data in random_points_per_cell:
    ...
    line_to_pos_start = (
        int(line_to_pos.coords.xy[0][0]) - 35917,
        int(line_to_pos.coords.xy[1][0]) - 23945,
    )
    line_to_pos_end = (
        int(line_to_pos.coords.xy[0][1]) - 35917,
        int(line_to_pos.coords.xy[1][1]) - 23945,
    )
    line_to_neg = shapely.wkt.loads(point_data.line_to_marker_neg)
    line_to_neg_start = (
        int(line_to_neg.coords.xy[0][0]) - 35917,
        int(line_to_neg.coords.xy[1][0]) - 23945,
    )
    line_to_neg_end = (
        int(line_to_neg.coords.xy[0][1]) - 35917,
        int(line_to_neg.coords.xy[1][1]) - 23945,
    )

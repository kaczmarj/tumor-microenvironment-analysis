...
def main(
    ...
    for roi_parent in roi_root.glob("ROI_*"):
        roi_num = roi_parent.name
        print("Reading image ...")
        offset_path = roi_parent / "offset.txt"
        roi_path = roi_parent / "merged_image.png"
        ...
        assert cv2.imwrite(str(output_dir / f"{roi_num}-overlay-boundaries.png"), image)
        ...
        assert cv2.imwrite(
            str(output_dir / f"{roi_num}-overlay-boundaries-cells-lines.png"), image
        )


if __name__ == "__main__":
    p = argparse.ArgumentParser(description=__doc__)
    p.add_argument("--roi-root", type=Path, required=True)
